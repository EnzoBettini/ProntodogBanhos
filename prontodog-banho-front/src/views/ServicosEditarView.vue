      ...
      <form @submit.prevent="submeterFormulario" class="space-y-8">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div class="animate-slide-up">
                ...
                <div class="relative">
                  <select
                    v-model="formulario.nome"
                    class="w-full px-4 py-3 pr-12 border-2 border-amber-200 rounded-xl focus:border-amber-500 focus:ring-4 focus:ring-amber-500/20 transition-all duration-300 bg-white appearance-none cursor-pointer hover:border-amber-400"
                    @change="preencherDescricaoAutomatica"
                  >
                    <option value="">Selecione um tipo de serviço</option>
                    <optgroup label="🛁 Serviços Únicos">
                      <option value="Banho Simples">Banho Simples</option>
                      <option value="Banho e Tosa">Banho e Tosa</option>
                      <option value="Tosa Higiênica">Tosa Higiênica</option>
                      <option value="Tosa Completa">Tosa Completa</option>
                      <option value="Escovação de Pelos">Escovação de Pelos</option>
                      <option value="Corte de Unhas">Corte de Unhas</option>
                      <option value="Limpeza de Ouvidos">Limpeza de Ouvidos</option>
                      <option value="Escovação Dental">Escovação Dental</option>
                      <option value="Hidratação">Hidratação</option>
                      <option value="Perfume">Perfume</option>
                      <option value="Transporte">Transporte</option>
                    </optgroup>
                    <optgroup label="📦 Pacotes de Banhos">
                      <option value="Pacote Banho Simples">Pacote Banho Simples</option>
                      <option value="Pacote Banho e Tosa">Pacote Banho e Tosa</option>
        ...
        <div class="bg-gradient-to-r from-white via-green-50/30 to-emerald-50/20 rounded-2xl shadow-lg border border-green-200/50 overflow-hidden animate-fade-in-up">
          <div class="bg-gradient-to-r from-green-600 to-emerald-700 px-6 py-4">
            <h2 class="text-xl font-bold text-white flex items-center gap-3">
              <div class="w-8 h-8 bg-white/20 rounded-lg flex items-center justify-center">
                <FontAwesomeIcon :icon="['fas', 'tags']" class="text-white" />
              </div>
              Configuração do Pacote
            </h2>
            <p class="text-green-100 text-sm mt-1">Defina se é banho único ou pacote de banhos</p>
          </div>

          <div class="p-6 space-y-6">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
              <!-- Valor do Serviço/Pacote -->
              <div class="animate-slide-up">
                <label class="block text-sm font-medium text-gray-700 mb-2">
              ...
              <div class="animate-slide-up">
                ...
                <div class="relative">
                  <select
                    ...
                    class="w-full px-4 py-3 pr-12 border-2 border-green-200 rounded-xl focus:border-green-500 focus:ring-4 focus:ring-green-500/20 transition-all duration-300 bg-white appearance-none cursor-pointer hover:border-green-400"
                    :class="{ 'border-red-300 focus:border-red-500': erros.quantidade }"
                    required
                  >
                    <optgroup label="🔸 Serviço Único">
                      <option value="1">1 Banho (Serviço Único)</option>
                    </optgroup>
                    <optgroup label="📦 Pacotes Pequenos">
                      <option value="2">2 Banhos</option>
                      <option value="3">3 Banhos</option>
                    </optgroup>
                    <optgroup label="🏆 Pacotes Populares">
                      <option value="4">4 Banhos (Pacote Padrão)</option>
                      <option value="5">5 Banhos</option>
                      <option value="6">6 Banhos</option>
                    </optgroup>
                    <optgroup label="💎 Pacotes Grandes">
                      <option value="8">8 Banhos</option>
                      <option value="10">10 Banhos</option>
                    </optgroup>
                  </select>
                  <!-- Ícone de seta customizado -->
                  <div class="absolute inset-y-0 right-0 flex items-center pr-3 pointer-events-none">
                    <FontAwesomeIcon :icon="['fas', 'chevron-down']" class="text-green-500" />
                  </div>
                </div>
                <div class="flex justify-between items-center mt-1">
                  <span v-if="erros.quantidade" class="text-sm text-red-600">{{ erros.quantidade }}</span>
                  <span class="text-xs text-gray-500 ml-auto">
                    {{ formulario.quantidade == 1 ? 'Serviço único' : `Pacote de ${formulario.quantidade} banhos` }}
                  </span>
                </div>
              </div>
            </div>
              ...
              <div class="flex items-center justify-between">
                ...
                <div class="text-right">
                  <template v-if="formulario.quantidade > 1">
                    ...
                    <div class="text-sm text-gray-500">por banho</div>
                  </template>
                  <template v-else>
                    <div class="text-lg font-bold text-green-600">
                      R$ {{ formatarValor(formulario.valor) }}
                    </div>
                    <div class="text-sm text-gray-500">valor único</div>
                  </template>
                </div>
              </div>
            </div>
            ...
            <div>
              ...
              <ul class="space-y-2 text-sm text-orange-700">
                ...
                <li class="flex items-center gap-2">
                  ...
                  O valor é o preço total do pacote/serviço em reais (R$)
                </li>
                <li class="flex items-center gap-2">
...
const preencherDescricaoAutomatica = () => {
  ...
  if (!formulario.descricao && formulario.nome) {
    const descricoesPadrao: Record<string, string> = {
      ...
      // Pacotes de Banhos
      'Pacote Banho Simples': 'Pacote de múltiplos banhos simples com desconto especial.',
      'Pacote Banho e Tosa': 'Pacote completo com banho e tosa, ideal para manutenção regular.',
      'Pacote Premium': 'Pacote completo com todos os serviços: banho, tosa, hidratação e perfume.',
      'Pacote Completo': 'Pacote VIP com serviços completos e atendimento diferenciado.'
    }
